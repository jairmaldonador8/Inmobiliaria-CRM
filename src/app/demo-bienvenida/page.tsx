...
function ConceptoA() {
  ...
  const [nombre, setNombre] = useState(PRIMER_NOMBRE)
  const [telefono, setTelefono] = useState('')
  const [tieneFoto, setTieneFoto] = useState(false)

  // Nombre y WhatsApp son obligatorios; la foto NO detiene el avance.
  const faltanDatos = nombre.trim() === '' || telefono.trim() === ''
  const bloqueado = paso === 1 && faltanDatos

  function reiniciar() {
    setPaso(0)
    setNombre(PRIMER_NOMBRE)
    setTelefono('')
    setTieneFoto(false)
  }

  if (paso === 3) return <DashboardFalso onReiniciar={reiniciar} />
    ...
    >
      ...
      {paso === 1 ? (
        <div className="flex flex-1 flex-col justify-center">
          ...
          <button
            type="button"
            onClick={() => setTieneFoto(!tieneFoto)}
            className="mt-7 flex items-center gap-4 self-start text-left"
          >
            <span
              className={cn(
                'relative flex size-20 items-center justify-center rounded-full text-2xl font-semibold',
                tieneFoto
                  ? 'bg-slate-700 text-white ring-2 ring-white'
                  : 'bg-slate-800 text-white'
              )}
            >
              {tieneFoto ? <Check aria-hidden className="size-7" /> : ASESOR.iniciales}
            ...
            <span className="text-sm font-medium text-slate-700 underline underline-offset-4">
              {tieneFoto ? 'Cambiar foto' : 'Subir una foto'}
            </span>
          </button>

          {/* La foto no bloquea, pero se insiste: sin ella el aviso no se va. */}
          {!tieneFoto ? (
            <p className="mt-3.5 flex gap-2 rounded-xl border border-white/80 bg-white/55 px-3 py-2.5 text-xs leading-snug text-slate-600">
              <AlertCircle aria-hidden className="mt-px size-3.5 shrink-0 text-slate-500" />
              <span>
                Te falta tu foto. Puedes seguir sin ella, pero es la que ven tus
                compañeros y la que va en tu ficha — súbela en cuanto puedas.
              </span>
            </p>
          ) : null}
          ...
          <div className="mt-6 flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <p className="text-xs text-slate-500">
                A este número se ligan los leads que atiendes.
              </p>
            </div>
          </div>
        </div>
      ...
      {/* Sin salida: la bienvenida se completa. Solo la foto es opcional. */}
      <Button
        type="button"
        disabled={bloqueado}
...
const CONCEPTOS = [
  {
    id: 'a',
    nombre: 'Bienvenida',
    resumen: 'Toma la pantalla, 3 pasos, obligatoria, una sola vez.',
    favor: [
      'Momento memorable: es lo primero que ve del sistema.',
      'Sin salida: nombre y WhatsApp entran completos el día uno.',
      'Absorbe los 3 avisos que hoy compiten en el primer ingreso.',
    ],
    contra: [
      'Bloquea. Un asesor con prisa la va a sufrir — por eso son 3 pasos, no 6.',
      'Cada campo obligatorio que se agregue después encarece este muro.',
    ],
